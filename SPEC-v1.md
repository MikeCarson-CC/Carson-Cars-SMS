...
- Insurance outreach (v2)
- ProMax 8,000-customer re-engagement campaign (separate project)
- AI-initiated payment plan negotiation (escalate to Jessica instead)
- Real-time ACV / book value lookups (using payoff vs original loan as proxy)
- Twilio number rotation (one dedicated AR number only)
- Bridged click-to-call from Telegram (v2/v3)
- Spanish first-touch messages based on language preference (v2 once DP support confirmed)
- Auto-replies to non-STOP messages (all replies go through human review in v1)

---

## 17. Success Metrics (review 30 days after launch)

- **Primary:** dollars collected attributable to SMS program (portal clicks → payments within 24-48h)
- Reply rate
- Click rate
- Conversion rate (click → payment)
- Opt-out rate (target: under 5%)
- Delivery rate (target: 90%+)
- Average response time Jessica/Evelyn to reviewed drafts
- Jessica's qualitative read: "Is this actually making me faster, or more work?"

**If the system isn't clearly generating ROI at 30 days, review and iterate — do not sunset before tuning.**
